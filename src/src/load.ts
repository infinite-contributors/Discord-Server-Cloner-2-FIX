...
export const loadChannels = async (
        ...
        try {
          ...
          console.log(
            gradient(["#ff4500", "#ffa500", "#ff6347"])(
              t("categorycreate") + createdCategory.name
            )
          );
          await Promise.all(
            categoryData.children.map(
              async (channelData: TextChannelData | VoiceChannelData) => {
                try {
                  await loadChannel(
                    channelData,
                    guild,
                  ...
                  console.log(
                    gradient(["#43a1ff", "#8a3ffc", "#3c0080"])(
                      t("voicechannelcreate") + channelData.name
                    )
                  );
                } catch (error) {
                  console.error(
                    `Error loading channel ${channelData.name}:`,
                    error
                  );
                }
              }
  ...
  backupData.channels.others.forEach(
...
export const loadEmojis = (
  ...
  backupData.emojis.forEach((emoji) => {
    if (emoji.url) {
      ...
      console.log(
        gradient(["#ff4500", "#ffa500", "#ff6347"])(
          t("emojicreate") + emoji.url + ", " + emoji.name
        )
      );
      ...
      console.log(
        gradient(["#ff4500", "#ffa500", "#ff6347"])(
          `Emoji criado com base64, Nome: ${emoji.name}`
        )
      );
    }
  });
